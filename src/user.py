...
class BiliUser:
    ...
    async def sendDanmaku(self):
        ...
        if not self.config['DANMAKU_CD']:
            ...
            return
        self.log.log("INFO", "弹幕打卡任务开始....(预计 {} 秒完成)".format(len(self.medals) * self.config['DANMAKU_CD'] * 11))
        n = 0
        successnum=0
        for medal in self.medals:
            n += 1
            ...
            for i in range(11):
                try:
                    ...
                    #     "DEBUG",
                    #     "{} 房间弹幕第{}次打卡成功: {} ({}/{})".format(
                    #         medal['anchor_info']['nick_name'], i+1, danmaku, n, len(self.medals)
                    #     ),
                    # )
                except Exception as e:
                    self.log.log("ERROR", "{} 房间第{}次弹幕打卡失败: {}".format(medal['anchor_info']['nick_name'], i, e))
            ...
            self.log.log(
                "DEBUG",
                "{} 房间弹幕共打卡成功{}次 ({}/{})".format(
                    medal['anchor_info']['nick_name'], cnt, n, len(self.medals)
                ),
            )
            if cnt>=10:
                successnum+=1
    ...
    async def sendmsg(self):
        ...
        for medal in self.medals:
            ...
            elif 150 <= today_feed <= 200:
                nameList2.append(nick_name)
            elif 70 <= today_feed <= 150:
